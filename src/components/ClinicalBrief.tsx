import type { AnalysisResult, Finding } from "@/lib/cpic/types";
import type { DatasetMeta } from "@/components/ProvenanceFooter";
import { classificationLabel, populationClinical } from "@/lib/present";

export interface BriefSubject {
  /** Patient identifier line. Omitted for an anonymous upload. */
  name?: string;
  /** Why they are on these drugs. */
  indication?: string;
  /** Where the genotype came from. */
  source?: string;
}
 ...
 * prose the reader has to parse for meaning. Actionable findings sit at the top
 * in a fixed-column table so the eye can run down the action column alone; the
 * full panel and provenance sit below the fold of attention, present for anyone
 * who wants to check the work.
 *
 * Takes its result as a prop rather than computing it, so the same component
 * serves the worked example and a freshly analysed upload. Before this, the
 * brief always rendered the demo patient — meaning anyone who analysed their own
 * data and opened the brief would have been shown someone else's genotypes
...
export function ClinicalBrief({
  ...
  meta: DatasetMeta;
  /** ISO date. Passed in so server and client renders cannot disagree. */
  generatedOn: string;
}) {
    ...
    <>
        ...
        <div className="-mx-6 overflow-x-auto px-6 print:mx-0 print:overflow-visible print:px-0">
          <table className="w-full min-w-[560px] border-collapse print:min-w-0">
            ...
            <tbody>
              {actionable.length === 0 ? (
                <tr>
                  <td colSpan={4} className="py-2 text-[12px] text-muted">
                    No actionable gene-drug interactions for this panel.
                  </td>
                </tr>
              ) : (
                actionable.map((f) => (
        ...
        <section className="mt-4">
          ...
          <p className="text-[12px] leading-relaxed text-foreground/80">
            {[
              ...routine.map(
                (f) =>
                  `${f.drugName} (${f.genotypes.map((g) => g.gene).join("/")}: standard dosing)`,
              ),
      ...
      <footer className="mt-5 border-t border-border pt-2.5 text-[10px] leading-relaxed text-muted">
        <p>
          ...
          text is reproduced verbatim; no generative model selects, edits or ranks clinical
          content. Recommendations requiring genes absent from this panel are not shown.
        </p>
        <p className="mt-1">
          <span className="font-semibold text-foreground/80">Dataset.</span> CPIC{" "}
          {meta.recommendations.toLocaleString()} recommendations · {meta.guidelines}{" "}
          guidelines · retrieved {meta.seededAt.slice(0, 10)}. Full guideline text at
          cpicpgx.org.
        </p>
        <p className="mt-1 font-semibold text-foreground/80">
          Demonstration prototype. Not a medical device. Not medical advice, diagnosis or
          treatment. Clinical decisions remain the responsibility of the treating
          clinician.
        </p>
      </footer>
    </>
